...
model_number = len(sys.argv) - 3 # 命令行参数输入测试模型的路径，可以同时有多个测试模型
vocab_name = sys.argv[1] # 词典，实际上是global向量
log_name = sys.argv[2] # 对于每一个model，其回答错误的问题将会被输入到对应的log文件中
model_name = []
for i in range(model_number) :
    model_name.append(sys.argv[i + 3]) # 所有model的文件名
...
def test(vocab, model, database) :
    total_number = 0
    total_correct = [0 for i in range(model_number)]
    part_number = 0
    part_correct = [0 for i in range(model_number)]
    log = []
    for i in range(model_number) :
        log.append(open(log_name + str(i) + "." + database, "w"))
    ...
    for line in open("./test/" + database + ".txt" , "r") :
        if (line[0] == ":") : # 如果到了一个新的测试类
            if part_number != 0 : # 如果这不是第一个测试类
                ...
                for i in range(model_number) :
                    print "        model %d correct number : " % i, part_correct[i]
                    print "        model %d accuracy : " % i, float(part_correct[i]) / part_number
            print line[:-1]
            total_number += part_number # 数据统计与更新
            part_number = 0
            for i in range(model_number) :
                total_correct[i] += part_correct[i]
                part_correct[i] = 0
            continue
        words = line.strip().lower().split() # 获取一组问题
        if (exist(vocab, words) == 0) : continue # 如果有词语不存在于词典中
        for i in range(model_number) : # 对于每个模型都进行测试
            if question(model[i], words) == 1 : part_correct[i] += 1 # 如果回答正确就更新
            else : log[i].write(words[0] + " " + words[1] + " " + words[2] + " " + words[3] + "\n") # 如果回答错误就输出到相应log文件中
        part_number += 1
    print "    questions number : ", part_number
    for i in range(model_number) :
        print "        model %d correct number : " % i, part_correct[i]
    ...
    for i in range(model_number) :
        print "    model %d correct number : " % i, total_correct[i]
        print "    model %d accuracy : " % i, float(total_correct[i]) / total_number
    print ""

vocab = gensim.models.KeyedVectors.load_word2vec_format(vocab_name) # 读取词典
print "loading vocabulary successfully!"
model = []
for i in range(model_number) : # 读取模型
    model.append(gensim.models.KeyedVectors.load_word2vec_format(model_name[i]))
    print "loading model %d successfully!" % i
